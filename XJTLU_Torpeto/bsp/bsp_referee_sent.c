...
void Command_send(void)
{
   ...
   framehead.SOF=Referee_SOF;
   framehead.Data_Length=Referee_Data_Length;
   framehead.Seq=UI_Seq;
   framehead.CRC8=Get_CRC8_Check_Sum_UI(framepoint,4,0xFF);
   framehead.CMD_ID=Robo_Exchange;                   //填充包头数据
   
   datahead.Data_ID= Send_command;
	if(robot_state.robot_id == Red_Launch_Dart)
	{
		datahead.Sender_ID= Red_Launch_Dart;
		datahead.Receiver_ID= Red_Sentry;
	}
	else if (robot_state.robot_id == Blue_Launch_Dart)
	{
		datahead.Sender_ID= Blue_Launch_Dart;
		datahead.Receiver_ID= Blue_Sentry;
	}                          //填充操作数据
   ...
   framepoint=(unsigned char *)&rc_ctrl;
   frametail=Get_CRC16_Check_Sum_UI(framepoint,sizeof(rc_ctrl),frametail);  //CRC16校验值计算
   ...
   framepoint=(unsigned char *)&rc_ctrl;
   for(loop_control=0;loop_control<sizeof(rc_ctrl);loop_control++)
   {
      UI_SendByte(*framepoint);
      framepoint++;
   }                                                                 //发送所有帧
